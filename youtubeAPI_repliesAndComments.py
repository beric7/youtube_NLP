...
def extract_comments_time(video_id, channel, video_title, video_desc, save_csv_location):
    comments_pop = []
    reply_count_pop = []
    like_count_pop = []
        ...
        while 1:
          ...
          for item in response['items']:
              if 'replies' in item.keys():
                  reply_text = item['replies']['comments'][0]["snippet"]["textDisplay"]
                  ...
                  if (count % 250) == 0:
                      print(count)
          if nextPage_token is None or count > 500:
            break
        
def extract_comments_relevance(video_id, channel, video_title, video_desc, save_csv_location):
    comments_pop = []
    reply_count_pop = []
    like_count_pop = []
...
def remove_emojis(csv_file):
    ...
    comments['clean_comment'] = comments['Comment'].apply(lambda x: demoji.replace(x,""))
    comments['clean_first_reply'] = comments['Initial Reply'].apply(lambda x: demoji.replace(x,""))
    return comments
...
def show_english_only(comments, save_location, english_comment_csv, query):
    ...
    for i in tqdm(range(0,len(comments))):
      ...
      temp = comments['clean_comment'].iloc[i]
      count += 1
      try:
...
def remove_special_characters(english_comment_csv):
    ...
    copy['reg'] = copy['clean_comment'].apply(lambda x:re.findall(regex,x))
    copy['regular_comments'] = copy['clean_comment'].apply(lambda x:re.sub(regex,"  ",x))
...
def create_dataset(english_comment_csv, dataset_destination):
    ...
    """
    en_comments = pd.read_csv(english_comment_csv, error_bad_lines=False, encoding='utf-8-sig')
    dataset = en_comments[['clean_comment', 'clean_first_reply','Replies', 'Likes']].copy()
...
if __name__ == '__main__':
    ...
    extract_comments_time(video_id, channel, video_title, video_desc, save_csv_t_location)
    extract_comments_relevance(video_id, channel, video_title, video_desc, save_csv_r_location)
    ...
    concat_df = pd.concat([csv_time,csv_rel])
    concat_df.to_csv(save_csv_location, index = False, line_terminator='\n', encoding='utf-8-sig')
    
    comments = remove_emojis(save_csv_location)
    unique_df = comments.drop_duplicates(subset=['Comment'])
    english_comm = show_english_only(unique_df, save_location, english_comment_csv, query)
