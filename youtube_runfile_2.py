...
# extract_comments_with_replies(video_id, channel, video_title, video_desc, save_csv_t_location, number, 'time', service)
# extract_comments_with_replies(video_id, channel, video_title, video_desc, save_csv_r_location, number, 'relevance', service)
...
df, regularized_reply_list = regularize(df, 'lowercase_reply', 'new_regularized_reply', regex)

print('spacing...')

df, spaced_comment_list = add_space(df, 'new_regularized_comment', 'spaced_comment', char_list)
df, spaced_reply_list = add_space(df, 'new_regularized_reply', 'spaced_reply', char_list)

print('reomve multi-spacing...')
df, no_multi_spaced_comment_list = remove_multi_space(df, 'spaced_comment', 'no_space_comment')
df, no_multi_spaced_reply_list = remove_multi_space(df, 'spaced_reply', 'no_space_reply')

print('histograms...')
most_common_comments, unique_comment_word_list = most_common_histogram(df, 'no_space_comment', 30, char_list)
plot_histogram(most_common_comments, 'Common words found in comments for Childish Gambino')

most_common_replies, unique_reply_word_list = most_common_histogram(df, 'no_space_reply', 30, char_list)
...
old_dataset_location = 'C://Users/Eric Bianchi/Desktop/csv_files/Childish Gambino - \
This Is America (Official Video)_cleaned_dataset.csv'
dataset_destination = 'C://Users/Eric Bianchi/Desktop/csv_files/Childish Gambino - \
This Is America (Official Video)_refined.csv'

old_dataset = pd.read_csv(old_dataset_location, error_bad_lines=False, encoding='utf-8-sig')
dataset = old_dataset[['no_space_comment','no_space_reply','Replies', 'Likes']].copy()
